...
Private Declare Function LoadImage Lib "user32.dll" Alias "LoadImageA" (ByVal hInst As Long, ByVal lpszName As Long, ByVal uType As Long, ByVal cxDesired As Long, ByVal cyDesired As Long, ByVal fuLoad As Long) As Long
'HANDLE LoadImage(
...
'    icleft As String 'Icon left
'    ictop As String 'Icon top
    icleft As Long 'Icon left
    ictop As Long 'Icon top
    sx As String 'Start X
    sy As String 'Start Y
...
If nColumns > 0 And nRows > 0 Then 'CREATE OR ADDTO IMAGELIST
    ...
    If hImageList = 0 Then hImageList = ImageList_Create(IconSize, IconSize, 32 + 1, 0, 0)  'ILC_MASK = &H1&
 ...
 If icIndex < 1 Or icIndex = n Then
        BitBlt dc, 0, 0, IconSize, IconSize, dc0, x, Y, &HCC0020   'SRCCOPY = &HCC0020
...
'Case menu16: i = 2 'icar = menu16ar
''If index > 999 Then himl=sys16
'Case sm9: i = 3 ' icar = sm9ar
...
If ArrayDims(icar(i)) <> 1 Then icar(i) = Array()
...
Else 'Restore
    If ii Then ImageList_AddIcons menu16, MyPic, 13, 13, 16, 30, 9, 19, 19, , ii
End If

Dim a: If IsArray(icar(2)) Then For h = IIf(ii < 1, 0, ii) To IIf(ii < 0, UBound(icar(2)), ii): DestroyIcon icar(2)(h): icar(2)(h) = 0: Next
...
If (flag And 1) = 1 Then
    If INIT And (comGDI.flag And 1) = 0 Then
        ...
        comGDI.Brush_Gray = GlobalBrush(&H808080, bc)
...
ElseIf ArrayDims(v) = 1 Then 'Load SysColors from array
...
hi = i: pn = 0
Select Case i
Case 1, 2, 3, 4, 5: s = Choose(i, "<<", "<", Format$(cPos, "mmmm yyyyг."), ">", ">>") 'Кнопки + Месяц год
    If i = 3 Then hi = -1 Else fn = comGDI.Font_Small: br = comGDI.Brush_Gray
Case 6, 7, 8, 9, 10, 11, 12: s = Choose(i - 5, "Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс") 'Дни недели
    hi = -1: fc = IIf(i < 11, &H808080, IIf(i = 11, &HFF6633, &H8080FF)) 'Черный светлосиний светлокрасный
Case 13, 21, 29, 37, 45, 53: s = Format$(d + 4, "ww", vbMonday, vbFirstFourDays) 'Номер недели
    hi = -1: fc = &HAA6666 '&HDAA5A5
    'fn = comGDI.Font_Small
    'br = 0 ' comGDI.Brush_LightGray
Case 61: s = "Сегодня " & Format$(Date, "dd.mm.yy") 'Кнопка сегодня
    br = comGDI.Brush_ToolTip
Case 62: s = "Отмена" 'Кнопка отмена
    br = comGDI.Brush_ToolTip
...
'================ PERIOD ==================
Public Function HitPeriod&(x&, Y&, index&, rc As RECT)
Dim r0 As RECT, i&
If index = 0 Then 'Надо вернуть ХИТ по указанным координатам x,y
...
If i > 0 And i < 6 Then 'Надо вернуть рект по указанному индексу
    ...
    ElseIf i = 3 Then
        'rc.Top = -1
        rc.Left = 64: rc.Right = 156
    ElseIf i = 4 Then
        rc.Left = 156: rc.Right = 202
    ElseIf i = 5 Then
        rc.Left = 204: rc.Right = 218
        rc.Top = 1: rc.Bottom = 12
    Else
        rc = r0
    End If
End If
End Function
Public Sub Redraw_Period(hDC&, cVal, cHit&, Optional index&, Optional OffsetX&, Optional offsetY&)
Dim i&, ar
Dim rc As RECT
Dim br&, fn&, s$
Dim b0&, f0&, fc&
...
'ЗАЩИТА ОТ ДУРАКА
On Error Resume Next
For i = IIf(index = 0, 1, index) To IIf(index = 0, 5, index)
HitPeriod 0, 0, i, rc
...
Case "IA": icon_align = gt3 'Global IconAlign 0 left+vtop 1 center ,2 right 4-vcenter 8 vbottom, 16=noMBdown
...
Case "FR" 'Frame RESTORE cr=sr(sri)
    sri = -Abs(gt3)
    'If UBound(sr) <= sri Then cr = sr(sri)
    'sri = -sri
...
Case "RX": rc = rc0: cr = rc: LSet w = w0 'ResetFormat
...
If Len(w.ff) Then hFont = GlobalFontSRC(lf, w.ff, 0): mLineHeight = 0 Else hFont = DFP.hFont
...
If Len(w.sx) Then If w.sx = 0 Then cr.Left = rc.Left Else If w.sx > 0 Then cr.Left = rc.Left + w.sx Else cr.Left = rc.Right + w.sx  'StartDrawX
If Len(w.sy) Then If w.sy = 0 Then cr.Top = rc.Top Else If w.sy > 0 Then cr.Top = rc.Top + w.sy Else cr.Top = rc.Bottom + w.sy  'StartDrawY
...
ElseIf sri >= 0 Then
    cr.Right = cr.Left + GetTextWidthHeight(hFont, "|" & txt, 1) + w.tr + w.tl
    If ic_sz And Len(txt) > 0 Then cr.Right = cr.Right + ic_sz + 3 '19 'Увеличиваем размер для значка
...
ElseIf sri >= 0 Then
    ...
    If Len(txt) Then
        cr.Bottom = cr.Bottom + mLineHeight * UBound(Split(txt, vbCrLf))
    End If
End If
...
'    End If
    SetBrushOrgEx xHDC, w.bx, w.by, pt
    oldhBrush = SelectObject(xHDC, hBrush)

End If
'SetBrushOrgEx xHDC, 0, 0, pt
'If Len(w.bg) Then
'    'If Not IsNumeric(w.bg) Then
'    SetBrushOrgEx xHDC, w.bx, w.by, pt
'    oldhBrush = SelectObject(xHDC, hBrush)
'End If
...
ElseIf Len(w.pc) Then 'Надо рисовать рамку с фоном
    ic = Val(w.pc)
    'If ic Then
        If ic < 0 Then ic = 0: If bCFrame Then ic = mOver    'HIGHLIGHT BORDER
        If w.pc = "0" Then ic = 1
        If ic Then
            oldhPen = SelectObject(xHDC, CreatePen(0, 1, AnyColor(Abs(w.pc))))
            If w.rr Then
...
Else 'Залифаем фон фрейма
    ...
    If Len(w.bg) Then FillRect xHDC, prc, 0        ' hBrush      'Заливаем фрейм
End If
...
If ic_i > 0 Then
    ic = Val(w.ic)
    If hit.Right > 0 And mOver And ((icon_align And 16) <> 16) Then OffsetRect prc, 1, 1
    If Len(w.df) Then prc.Left = prc.Left + 2 'OffsetRect prc, 2, 0
    If icon_align And 1 Then prc.Left = prc.Left + (prc.Right - prc.Left - ic_sz) / 2 'CENTER
    If icon_align And 2 Then prc.Left = prc.Right - ic_sz 'RIGHT
    If icon_align And 4 Then prc.Top = prc.Top + (prc.Bottom - prc.Top - ic_sz) / 2 'VCENTER
    If icon_align And 8 Then prc.Top = prc.Bottom - ic_sz 'VBOTTOM
...
'            End If
    ...
    If ic_i Then 'text with icon
        ...
        If icon_align And 2 Then trc.Right = trc.Right - ic_sz Else trc.Left = trc.Left + ic_sz
    End If
...
If Not DFP.Enabled Then
'EmbosseDC xHDC, rc
DisableDC xHDC, rc
'    hBackBrush = SelectObject(xHDC, comGDI.Brush_Gray)
'    BitBlt xHDC, rc.Left + 1, rc.Top + 1, rc.Right, rc.Bottom, xHDC, rc.Left + 1, rc.Top + 1, &HFC008A
'    hBackBrush = SelectObject(xHDC, hBackBrush)
End If
...
'Dim i&, n&, h&
'n = EnumSubClass.ParamCount - 1
'For i = 0 To n
'    h = EnumSubClass.Param(i)
...
Public Sub LookUpHord(ByVal hWndMaster&)
...
For i = 0 To UBound(ar) Step 2 'Групируем рабов
    ...
    If IsWindow(hWndSlave) Then gAddIndex sar, hWndSlave 'Список рабов
Next
...
If EqualRect(rcSlave, rc) = 0 Then  'Если ректы отличаются то двигаем
    SetWindowPos hWndSlave, 0, rc.Left, rc.Top, rc.Right - rc.Left, rc.Bottom - rc.Top, &H24 Or SWP_NOACTIVATE   ' Or &H400
End If
...
Else 'NAMEor BOLD or ITALIC
    ...
    ElseIf Len(a) = 0 And IsEmpty(f(0)) Then  'Name=Default
        ...
        f(0) = RTrimChar(StrConv(p.lfFaceName, vbUnicode))
...
If IsEmpty(f(0)) Then f(0) = RTrimChar(StrConv(p.lfFaceName, vbUnicode))
...
'LogFontSRC = RTrimNull$(StrConv(lf.lfFaceName, vbUnicode)) & "," & MulDiv(-lf.lfHeight, 72, 96) & IIf(lf.lfWeight >= 700, ",B", "") & IIf(lf.lfItalic, ",I", "") & IIf(lf.lfUnderline, ",U", "") & IIf(lf.lfStrikeOut, ",S", "") & IIf(lf.lfWidth, "," & lf.lfWidth, "")
LogFontSRC = RTrimChar(StrConv(lf.lfFaceName, vbUnicode)) & "," & MulDiv(-lf.lfHeight, 72, 96) & IIf(lf.lfWeight >= 700, ",B", "") & IIf(lf.lfItalic, ",I", "") & IIf(lf.lfUnderline, ",U", "") & IIf(lf.lfStrikeOut, ",S", "") & IIf(lf.lfWidth, "," & lf.lfWidth, "")
End Function
'================ F O N T ================================

Function GlobalPen&(PenColor&, Optional pw&)
Dim f$, n&, src$
'f = Hex$(TranslateColor(PenColor)) & IIf(pw > 0, "." & pw, "")
...
Else
'    If PenColor > 0 And PenColor < 32 Then
'        GlobalPen = CreatePen(0, pw, GetSysColor(PenColor))
'    Else
'        GlobalPen = CreatePen(0, pw, TranslateColor(PenColor))
'    End If
...
If IsNumeric(f) Then 'color
    bc = f
'    If bc < 0 Then bc = TranslateColor(bc)
'    If bc < 32 Then
'        GlobalBrush = GetSysColorBrush(bc)
'        back_color = GetSysColor(bc)
'    Else
'        GlobalBrush = CreateSolidBrush(bc)
'        back_color = bc
'    End If
...
Else 'string
    ...
    Else
        ...
        If Len(f) Then
            ...
            DrawFormat Nothing, 0, rc, f, RCP, dp 'Calc SIZE
            rc = RCP
            CreateMemDC hmemDC, RCP.Right, RCP.Bottom
            Dim hOldBmp&, hbmp&
            hbmp = CreateCompatibleBitmap(hmemDC, RCP.Right, RCP.Bottom)
            hOldBmp = SelectObject(hmemDC, hbmp)
            DrawFormat Nothing, hmemDC, rc, f, RCP, dp
